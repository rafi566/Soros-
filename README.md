...
- `GET /fanouts` – available one-to-many sync groupings from a single source to multiple destinations
- `POST /jobs` – start a long-running sync job with asynchronous progress tracking (accepts `sourceId` and `destinationIds`)
